...
private:
    ...
    pair<int, long long> countAndSumSubarrays(const vector<int>& array, long long threshold) {
        int count = 0;
...
public:
    int rangeSum(vector<int>& nums, int n, int left, int right) {
        minSubarraySum = *min_element(nums.begin(), nums.end());
        maxSubarraySum = accumulate(nums.begin(), nums.end(), 0LL);
